...
order_factor <- function(fcc_data_var, rev) {
...
# Pretty boxplot
nice_boxplot <- function(df, xlab, ylab, question) {
  colnames(df)[1] <- "x"
  colnames(df)[2] <- "y"
  ...
  # Not using a sequential scale_fill_brewer palette here
  # Don't want to imply superiority order along x-axis
  # Order of categories used to convey number of respondents
...
# y-axis = exp, x-axis = , legend color = hours_learning.bucket (needs to be sequential)

# x and y axes should be numerical. see panel for vars

table(fcc_data$MaritalStatus)
```
...
# say no final plots section because i polished all of them for kaggle (say it right before where section start would be)