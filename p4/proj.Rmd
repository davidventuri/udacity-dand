...
nice_bar_plot <- function(col, question, num_answers, lab_size, lab_cut) {
  ...
  plot <- ggplot(df) +
    geom_bar(aes(x, alpha=factor(x)), fill="#02b3e4") +
    theme_minimal() +
    scale_x_discrete(drop=FALSE) +
    labs(title="", x=NULL, y=paste(as.character(num_answers), "out of 646 developing data scientists/engineers answered")) +
    theme(axis.text=element_text(size=10),
          axis.title.x=element_text(colour="#02b3e4", size=10, margin=margin(15,0,9,0)), legend.position="none") +
...
g_code_event <- grid.arrange(nice_bar_plot(droplevels(code_event_factor_lim$event), "Which types of in-person coding events have you attended?", num_answers_code_event, 5.2, 5))
...
Only 11 of the 954 respondents that attended a bootcamp in the full new coder survey are pursuing data science/engineering.
...
67% have earned at least a bachelor’s degree. Compared to 58% for the full new coder survey, the data-focused subset is more skewed towards graduate studies.
