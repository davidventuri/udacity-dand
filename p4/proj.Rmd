...
#with(non_fcc_data, cor.test(Age, MonthsProgramming, method = 'pearson'))

# this supports that there isn't one main feature
pp1 <- pairs.panels(select(fcc_data, Age, MonthsProgramming, HoursLearning, Income, ExpectedEarning, StudentDebtOwe), hist.col="#377bb5", col="#FF4541")

pp2 <- pairs.panels(select(non_fcc_data, Age, MonthsProgramming, HoursLearning, Income, ExpectedEarning, StudentDebtOwe), hist.col="#377bb5", col="#FF4541")

# talk about what the fitting thing is
...
# Others correlate heavy, but they aren't that illuminating with regards to data science specific.

# Hours Learning by Gender

# Hours Learning by ContinentLive

# Expected Earning by Gender
ggplot(fcc_data, aes(x=Gender, y=ExpectedEarning, fill=Gender)) +
  geom_boxplot() +
  theme_minimal()

# Expected Earning by ContinentLive
ggplot(fcc_data, aes(x=ContinentLive, y=ExpectedEarning, fill=ContinentLive)) +
  geom_boxplot() +
  theme_minimal()
...
ggplot(fcc_data, aes(x=hours_learning.bucket, y=ExpectedEarning, fill=hours_learning.bucket)) +
  geom_boxplot() +
  theme_minimal()
```
