...
```{r echo=FALSE, message=FALSE, Learner_Goals_Approaches}
# JobPref
...
# EmploymentStatus
fcc_data$EmploymentStatus <- as.character(fcc_data$EmploymentStatus)
# Change "NA" to "other" if respondent provided EmploymentStatusOther
fcc_data$EmploymentStatus <- with(fcc_data, ifelse(is.na(EmploymentStatus) & !is.na(EmploymentStatusOther), 'Other', EmploymentStatus))
fcc_data$EmploymentStatus <- as.factor(fcc_data$EmploymentStatus)
...
g_employment_status <- grid.arrange(nice_bar_plot(employment_status_na_rm$EmploymentStatus, "Regarding employment status, are you currently...", nrow(employment_status_na_rm), 6, 4))
