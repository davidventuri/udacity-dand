...
#### <span style="color:#377bb5">The following analysis explores the characteristics of these developing data scientists/engineers and new coders in general.</span>
...
Interesting. I bet the skew towards post-secondary studies for the data science subset plays a role here, where higher levels of student debt and higher salaries are expected.
...
**Let's examine the full new coder survey.** One of the strongest correlations is between age and current salary. Note that this strength exists despite the majority of $200k salaries belonging to respondents under 40.
...
nice_scatter <- function(xcol, ycol, xlab, ylab) {
  ...
  geom_jitter(alpha = 0.50, color = "#377bb5") +
...
The earnings vs. age trend isn't maintained as these individuals prepare to transition to their new field of choice. Younger individuals appear willing to capitalize on lucrative tech salaries and older individuals appear willing to take a pay cut.
...
**The horizontal line is the median. The "x" is the mean.**
...
<br>
Not much differentiation here as well. All continents have a median of 10 hours dedicated to learning per week. Asian and African students have the highest means, at 16.4 and 16.8 hours, respectively.
...
<br>
Females expect higher salaries than males, with a $9k gap in medians and a $4k gap in means. There is a huge gap in first quartiles, where the 25th percentile female expects $14k more than her male equivalent. As with hours dedicated to learning, transgender new coders have relatively higher expected salaries.
...
Again, with the million dollar student debt outlier removed, ... mobile dev outlier drags up box

```{r Stud_Debt_Job_Role_Interest, fig.width = 8, fig.height = 6.5}
# Expected Earning by Job Role Interest
fcc_job_lim_sd_lim <- with(fcc_job_lim, fcc_job_lim[StudentDebtOwe < 1000000, ])
g_stud_debt_job_int <-
  grid.arrange(nice_boxplot(select(fcc_job_lim_sd_lim, JobRoleInterest, StudentDebtOwe),
                            "JobRoleInterest", "StudentDebtOwe (k)",
                            "About how much do you owe in student debt loans in USD?\nWhich one of these roles are you most interested in?",
                            c(290, 330)))
with(fcc_job_lim, by(StudentDebtOwe, JobRoleInterest, summary))
```
