...
I created five new variables, *ContinentCitizen*, *ContinentLive*, *PodcastPartiallyDerivative*, *PodcastBecomingDataSci*, and *PodcastTalkingMachines* for a new total of **118 variables**.
...
plot_title <- function(question) {
  title.grob <- textGrob(
    ...
    y = unit(-0.3, "lines"),
    hjust = 0, vjust = 0,
    gp = gpar(fontsize = 14, fontface="bold", col="#02b3e4"))
  return(title.grob)
}
...
nice_histogram <- function(col, bw, xlab) {
  ...
  plot <- ggplot(aes(x), data = df) +
    geom_histogram(binwidth=bw, color = '#02b3e4', fill = '#02b3e4') +
    theme_minimal() +
    labs(title="", x=paste("\n",xlab), y="Count\n") +
...
nice_bar_plot <- function(col, question, num_answers, lab_size, lab_cut) {
  ...
  plot <- ggplot(df) +
    geom_bar(aes(x, alpha=factor(..count..)), fill="#02b3e4") +
    theme_minimal() +
    ...
    theme(axis.text=element_text(size=10),
          axis.title.x=element_text(colour="#02b3e4", size=10,
...
g_months_prog <- grid.arrange(nice_histogram(months_prog_na_rm$MonthsProgramming/12, 0.25, "Years Programming"), top = months_prog.grob)
...
Less than 20% listen to coding-related podcasts. Of them, Partially Derivative, Becoming A Data Scientist, and Talking Machines are the only domain-specific podcasts noted.
...
47% consider themselves under-employed (working a job that is below their education level).

```{r echo=FALSE, message=FALSE, IsUnderEmployed}
# IsUnderEmployed
is_under_emp <- table(fcc_data$IsUnderEmployed)['1']
names(is_under_emp) = c("is under employed")
...
# HomeMortgageOwe
mean(fcc_data$HomeMortgageOwe, na.rm = TRUE)
home_mort_owe.grob <- plot_title("About how much do you owe on your home mortgage in US Dollars?")
...
# StudentDebtOwe
mean(fcc_data$StudentDebtOwe, na.rm = TRUE)
stud_debt_owe.grob <- plot_title("About how much do you owe in student debt loans in US Dollars?")
...
Removing the million dollar outlier, the distribution is much more clear with the majority of debt under $100k. I hope that outlier is a joke.
...
These are the people who are learning data science and engineering. Free, self-paced learning resources are clearly important.
